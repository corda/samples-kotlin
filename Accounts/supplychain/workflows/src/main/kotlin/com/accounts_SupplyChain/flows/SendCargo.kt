...
class SendCargo(
    ...
    override fun call(): String {
        ...
        //Create a key for transaction
        progressTracker.currentStep =GENERATING_KEYS
        val myAccount = accountService.accountInfo(pickupFrom).single().state.data
        ...
        transactionBuilder.addOutputState(output)
                .addCommand(CargoStateContract.Commands.Create(), listOf(targetAcctAnonymousParty.owningKey,ourIdentity.owningKey))
        progressTracker.currentStep =SIGNING_TRANSACTION
        val locallySignedTx = serviceHub.signInitialTransaction(transactionBuilder, listOfNotNull(ourIdentity.owningKey))
