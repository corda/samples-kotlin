...
class StockContract : EvolvableTokenContract(),Contract {
    ...
    override fun verify(tx: LedgerTransaction) {
        val outputState = tx.getOutput(0) as StockState
    ...
    override fun additionalCreateChecks(tx: LedgerTransaction) { // Number of outputs is guaranteed as 1
        val createdStockState = tx.outputsOfType<StockState>()[0]
        requireThat {
            //Validations when creating a new stock
            "Stock symbol must not be empty".using(createdStockState.symbol.isNotEmpty())
            "Stock name must not be empty".using(createdStockState.name.isNotEmpty())
    ...
    override fun additionalUpdateChecks(tx: LedgerTransaction) { // Number of inputs and outputs are guaranteed as 1
        ...
        requireThat {
            //Validations when a stock is updated, i.e. AnnounceDividend (UpdateEvolvableToken)
