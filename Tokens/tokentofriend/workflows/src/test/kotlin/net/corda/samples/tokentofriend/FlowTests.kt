...
class FlowTests {
    ...
    fun `Check the correct message is stored`() {
        ...
        val inputCriteria = QueryCriteria.LinearStateQueryCriteria(
            uuid = listOf(tokenStateID.id),
            status = Vault.StateStatus.UNCONSUMED
        )
        val storedState = a.services.vaultService.queryBy(
            CustomTokenState::class.java,
    ...
    fun `Check if non fungible token correctly created`() {
        ...
        val inputCriteria = QueryCriteria.LinearStateQueryCriteria(
            uuid = listOf(UUID.fromString(nonfungibleTokenId)),
            status = Vault.StateStatus.UNCONSUMED
        )
        val storedNonFungibleTokenb =
            b.services.vaultService.queryBy(NonFungibleToken::class.java, criteria = inputCriteria).states
        val storedNonFungibleTokenc =
            c.services.vaultService.queryBy(NonFungibleToken::class.java, criteria = inputCriteria).states
        val storedNonFungibleTokend =
            d.services.vaultService.queryBy(NonFungibleToken::class.java, criteria = inputCriteria).states
        val storedNonFungibleTokene =
            e.services.vaultService.queryBy(NonFungibleToken::class.java, criteria = inputCriteria).states

        val storedToken = listOf(storedNonFungibleTokenb, storedNonFungibleTokenc, storedNonFungibleTokend, storedNonFungibleTokene)
            .single { it.isNotEmpty() }.single().state.data
