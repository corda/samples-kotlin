...
class ApproveSyndicateBidTest {
    ...
    fun `Create Syndication flow Test`() {
        ...
        network.runNetwork()
        val loanBid =
            borrower.services.vaultService.queryBy(LoanBidState::class.java, inputCriteria).states[0].state.data
        ...
        network.runNetwork()
        val syndication =
            bankB.services.vaultService.queryBy(SyndicateState::class.java, inputCriteria).states[0].state.data

        //participating bank submit syndication bid
        val syndicationID = syndication.linearId
        val submitSyndicationBid = SubmitSyndicateBid(syndicationID, 2)
        val future5: Future<SignedTransaction> = bankB.startFlow(submitSyndicationBid)
        network.runNetwork()
        val syndicatedBid = bankA.services.vaultService.queryBy(SyndicateBidState::class.java, inputCriteria).states[0].state.data

        //Lead bank approves syndication bid
        val syndicationBidId = syndicatedBid.linearId
        val approveSyndicationBid = ApproveSyndicateBid(syndicationBidId)
        val future6: Future<SignedTransaction> = bankA.startFlow(approveSyndicationBid)
        network.runNetwork()
        val syndicatedBidApproved = bankA.services.vaultService.queryBy(SyndicateBidState::class.java, inputCriteria).states[0].state.data
    }
}